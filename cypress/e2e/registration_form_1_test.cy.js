...
describe('This is first test suite Lilia Parind', () => {
    it('User can submit data only when valid mandatory values are added', () => {
    // Date: 17.06.2023
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Lilia')
        cy.get('#lastName').type('Parind')
        cy.get('input[name="password"]').type('Lilitka12345')
        cy.get('[name="confirm"]').type('Lilitka12345')
    ...
    it('User cannot add letters to phone number', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('black')
